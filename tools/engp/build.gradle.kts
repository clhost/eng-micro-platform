...
application {
    applicationName = "engp"
    mainClass.set("io.clhost.tooling.engp.EngKt")
}

nativeImage {
    graalVmHome = System.getenv("GRAALVM_HOME") ?: System.getenv("JAVA_HOME")
    executableName = "engp"
    buildType { build -> build.executable(main = "io.clhost.tooling.engp.EngpKt") }
    arguments {
        add("--no-fallback")
